...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.content == "!tag" || message.content == "tag" || message.content == ".tag" || message.content == "/tag") {
      message.channel.send(config.bot.tag)
    }
    if (message.content == "!link") {
      message.channel.send("" + config.bot.serverlink + "<@" + message.author.id + ">")
    }
    ...
    if (message.channel.id == config.channel.globalchat) {
    ...
    let prefikslerim = ["j!", "!", "D!", "d.", "."];
    let jalixcim = false;
    for (const içindeki of prefikslerim) {
      if (message.content.startsWith(içindeki)) jalixcim = içindeki;
    }

    if (!jalixcim) return;

    const args = message.content
      .slice(jalixcim.length)
      .trim()
      .split(/ +/g);
    ...
    if (!owners.includes(message.author.id)) {
      ...
      if (blockArr.length == 9) {
        ...
        this.client.channels.cache.get(config.logchannels.commandlog).send(`**${message.author.tag}** - ${message.author}(\`${message.author.id}\`) komut engeli yedi.Komut kullanım özeti:\n\`\`\`${blockArr.map(x => x.içerik).join("\n")}\nKullandığı komutlar: ${blockArr.map(x => x.komut).join(",")}\nKullandığı kanallar: ${blockArr.map(x => x.kanal).join(",")}\`\`\``)
