...
class Booster extends Command {
    ...
    async run(message, args, data) {
        if(!message.member.roles.cache.has("729122774873800741")) return
        if(message.member.roles.cache.has("773271657582166086")) return
      ...
      message.author.username.includes("☨") ? await message.member.setNickname(`☨ ${args.slice(0).join(" ")}`) : await message.member.setNickname(`✦ ${args.slice(0).join(" ")}`)
      await message.react("772898104650498068")
    }
}
