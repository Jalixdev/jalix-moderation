...
class Cezalı extends Command {
    ...
    async run(message, args, level) {
		if (!message.member.roles.cache.some(r => this.client.config.ytrole.jailhammers.includes(r.id)) && !message.member.hasPermission("VIEW_AUDIT_LOG")) return;
        ...
        let id = await sunucu.findOne({ guild: this.client.config.bot.ServerID }).then(res => res.ihlal)
        ...
        if (!message.guild.members.cache.has(user.id)) {
            ...
            await cezalar.findOne({ user: user.id }, async (err, doc) => {
                ...
                let sonrakii = id + 1
                member.roles.set(member.roles.cache.has(this.client.config.role.booster) ? [this.client.config.role.booster, this.client.config.cezarole.karantina] : [this.client.config.cezarole.karantina]).catch(e => console.log(e))
                this.client.yolla("<@" + user + "> üyesine <@&"+ config.cezarole.karantina +"> rolü verildi. (Ceza Numarası: `#" + sonrakii + "`)", message.author, message.channel)
                const başe = new Discord.MessageEmbed()
                    .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                    .setColor("RANDOM")
                    .setDescription(`${user} üyesine __${args.slice(1).join(" ")}__ sebebi ile <@&`+ config.cezarole.karantina +`> rolü verildi.`)
                    .setFooter(`${moment(Date.parse(new Date().toLocaleString("tr-TR", { timeZone: "Asia/Istanbul" }))).format("LLL")}`)
                await this.client.channels.cache.get(this.client.config.logchannels.jailLogs).send(başe)
                if (!doc) {
                    const newPun = new cezalar({
                        user: user.id,
                        ceza: true,
